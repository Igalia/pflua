...
local set = require("pf.utils").set
...
local function IPProtocol()
   ...
   return choose({ "icmp", "igmp", "igrp", "pim", "ah", "esp", "vrrp",
                    "udp", "tcp", "sctp", "icmp6" })
   -- Ignore sctp, because "keyword elision not implemented sctp"; bug 137
end
...
local function packetAccessLocation()
   ...
   if r1 < 0.9 then base = uint8()
   else base = uint16()
   end
   return base
--[[ -- Commented out due to bug 139
   if r2 < 0.5 then
      return base
   else
      -- tcpdump only allows the following 3 offsets
      local offset = choose({1,2,4})
      return base .. ':' .. offset
   end
--]]
end
...
local function PacketAccess()
   ...
   -- Avoid two sets of protocols: ones libpcap doesn't allow, and pflua bugs
   -- libpcap does not allow 'ah' and 'esp' packet access; not a pflua bug
   -- Work around bug 138: igmp, pim, igrp, vrrp
   local skip_protos = set('ah', 'esp') --, 'igmp', 'pim', 'igrp', 'vrrp')
   ...
   return {pkt_access, comparisonOp(), uint8()}
end
